...
async def get_embedding(text):
    if not API_KEY:
        raise HTTPException(500, "API_KEY missing")
...
async def find_similar_content(embedding):
    conn = get_db_connection()
    cur = conn.cursor()
    results = []

    for table in ["discourse_chunks", "markdown_chunks"]:
        cur.execute(f"SELECT content, url, embedding FROM {table} WHERE embedding IS NOT NULL LIMIT 1000")
...
async def generate_answer(question, results):
    if not API_KEY:
        raise HTTPException(500, "API_KEY missing")
...
async def query(request: QueryRequest):
    try:
        ...
        results = await find_similar_content(embedding)
        if not results:
            return {"answer": "No relevant content found.", "links": []}

        reply = await generate_answer(request.question, results)
...
@app.get("/")
def root():
    return {"message": "Optimized TDS Virtual TA is live"}
...
def health():
    try:
        ...
        discourse_count = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM markdown_chunks")
        markdown_count = cur.fetchone()[0]
        conn.close()
        return {"status": "ok", "discourse": discourse_count, "markdown": markdown_count}
    except Exception as e:
        return JSONResponse(status_code=500, content={"error": str(e)})

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("optimized_app:app", host="0.0.0.0", port=8000)
...
# @app.get("/")
# def root():
#     return {"message": "TDS project 1 - Virtual TA"}
